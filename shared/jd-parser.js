...
export class JDParser {
  ...
  extractRequiredSkills(text) {
    ...
    const techRe = /\b(React(?:\.js)?|Vue(?:\.js)?|Angular|Next\.js|Nuxt(?:\.js)?|Svelte(?:Kit)?|Remix|Astro|Node(?:\.js)?|Express(?:\.js)?|NestJS|Fastify|Django|Flask|FastAPI|Spring(?:\s*Boot)?|Laravel|Rails|Ruby\s+on\s+Rails|Python|TypeScript|JavaScript|Go(?:lang)?|Rust|Java|C#|\.NET|PHP|Swift|Kotlin|Scala|Elixir|GraphQL|REST(?:ful)?|gRPC|tRPC|SQL|PostgreSQL|Postgres|MySQL|MongoDB|Redis|Elasticsearch|Kafka|RabbitMQ|AWS\s*CDK|AWS|GCP|Azure\s*ML|Azure|Docker|Kubernetes|K8s|Terraform|CI\/CD|GitHub\s*Actions|GitLab\s*CI|Git(?:Hub|Lab)?|Jira\s*Product\s*Discovery|Jira|Linear|Productboard|Aha!?|Pendo|Amplitude|Mixpanel|Miro|FigJam|Figma|Confluence|Notion|Linux|Bash|Shell|Kubeflow|KFP|KubeRay|Ray(?:\.io|\.Serve|\.Train)?|SkyPilot|DCGM|MLflow|DVC|Seldon|BentoML|Triton(?:\s+Inference\s+Server)?|Feast|WandB|Weights\s*&\s*Biases|ZenML|Metaflow|ClearML|Argo\s*(?:Workflows?|CD|Rollouts?)?|ArgoCD|Vertex\s*AI|SageMaker|ONNX|Dask|Polars|Evidently(?:\s*AI)?|Neptune|vLLM|Prefect|Dagster|Delta\s*Lake|Apache\s*(?:Spark|Kafka|Flink|Airflow|Iceberg)|Great\s*Expectations|Pulumi|Crossplane|OpenTelemetry|Prometheus|Grafana|Datadog|Istio|Helm|Vault|Consul|Flux(?:CD)?|Karpenter|KEDA|Kuberay|LangChain|LangGraph|LlamaIndex|Pinecone|Weaviate|Qdrant|ChromaDB|OpenAI|Anthropic|Bedrock|Vertex|HubSpot(?:\s*CRM)?|Marketo|Mailchimp|Klaviyo|SEMrush|Ahrefs|Google\s*Analytics|GA4|Google\s*Ads|Meta\s*Ads|LinkedIn\s*Ads|Hootsuite|Buffer|Sprout\s*Social|Webflow|WordPress|Salesforce|Gainsight|Totango|Intercom|Zendesk|Freshdesk|Gong|Outreach|Salesloft|ChurnZero|Catalyst|Planhat|Excel|Google\s*Sheets|Xero|QuickBooks|Sage|NetSuite|SAP|Oracle\s*ERP|Workday|Adaptive\s*Planning|Anaplan|Hyperion|BambooHR|HiBob|Greenhouse|Lever|ServiceNow|Monday\.com|Asana|Smartsheet)\b/gi;
    let m;
